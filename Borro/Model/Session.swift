...
class Session{
    ...
    func mergeToken(string:String){
        users.document("TgBXpiVAxxd0ozhNJEhn").setData(["token":string], merge: true)
    }
    ...
    func documentToUser(document:DocumentSnapshot,completionHandler:@escaping(User?)->Void){
        if let data = document.data(){
            ...
            Storage.storage().reference(withPath: data[usersFirebase.imageRef.rawValue] as! String).getData(maxSize: 1 * 1024 * 1024) { (imagedata, err) in
                ...
                DispatchQueue.main.async {
                    ...
                    let user = User(userID: document.documentID, fullName: data[usersFirebase.fullName.rawValue] as! String, email: data[usersFirebase.email.rawValue] as! String, mobile: data[usersFirebase.mobile.rawValue] as! String, image: image, sellerBio: data[usersFirebase.sellerBio.rawValue] as! String)
    ...
    //returns user in completion handler from a given id
    
    func getUserByDocumentID(userID:String, completionHandler: @escaping (User?) -> Void){
    ...
    //updates a list with all items according to a search
    
    func getAllItems(completionHandler: @escaping ([Item]?)->Void){
    ...
    //updates a list with all items belonging to a user
    
    func getItemsByUser(userID:String, completionHandler: @escaping ([Item]?) -> Void){
    ...
    func getItemByID(itemID:String,completionHandler: @escaping (Item?,Error?) -> Void){
        items.document(itemID).getDocument { (snapshot, err) in
            ...
            if let snapshot = snapshot{
                completionHandler(self.returnItemFromQuerySnapshot(documentSingle:snapshot),nil)
            }
        }
    }
   ...
   func returnUser(UserID:String) -> User{
    ...
    var rUser:User = User(userID: "", fullName: "Unknown", email: "", mobile: "",image: nil, sellerBio: "")
